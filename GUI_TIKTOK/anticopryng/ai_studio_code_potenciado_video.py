...
def procesar_variaciones_multimedia(entrada_dir, salida_dir, crf=23, preset="medium", seed=None):
    ...
    for i, src_path in enumerate(media_files):
        ...
        _r_up    = get_random_value(1.02, 0.0099)
        _r_down  = get_random_value(0.98, 0.0099)
        _r_combo = get_random_value(1.015, 0.0099)
        _t_rap   = get_random_value(1.02, 0.0099)
        _t_len   = get_random_value(0.98, 0.0099)
        _t_combo = get_random_value(1.015, 0.0099)
        recetas_audio_mejoradas = [
            ...
            {"name": "combo_aleatorio","af": f"atempo={_t_combo},asetrate=44100*{_r_combo},aresample=44100,atempo={round(1/_r_combo, 4)}", "vf_extra": f"setpts=PTS/{_t_combo}"},
        ]
        ...
        if extension == ".mp4":
            ...
            vf_chain = f"{_vf_pre + ',' if _vf_pre else ''}setsar=1,{geo['vf']},{color['vf']}"
            if ruido_vid["vf"]:
            ...
            if os.path.exists(ruido_path) and aplicar_ruido:
                ...
                if encoder == "h264_nvenc":
                    cmd = [
                        ...
                        "-map", "[v]", "-map", "[a]", "-c:v", "h264_nvenc", "-preset", "p4", "-cq", str(crf),
