...
if MODO == "hyper_tiktok":
    ...
    # 1. Video original intacto escalado al 65% de la pantalla (720x832) - VERSIÓN ACHATADA QUE FUNCIONA
    fc_parts.append("[0:v]scale=720:832,setsar=1[v_concat];")
    # Audio original
    fc_parts.append("[0:a]anull[outa];")
    
    # 2. Fondo Matemático al 35% (720x448) - Oscurecido y desaturado
    fc_parts.append(
        f"mandelbrot=size=720x448:rate=30,trim=duration={duracion_total},format=yuv420p,eq=saturation=0.1:brightness=-0.4[plasma];"
    )
    
    # 3. Unir Video Intacto (Arriba) con Plasma (Abajo)
    fc_parts.append("[v_concat][plasma]vstack=inputs=2[split_screen];")
    
    # 4. Overlays Reactivos (Simulación de GIFs con cuadros de colores)
    # ¡ESTA ES LA VERSIÓN QUE ENGAÑA A LA IA! Usar GIFs externos reales hace que
    # TikTok detecte el GIF en sí mismo como "Contenido No Original".
    overlay_filters = "[split_screen]"
    
    cmd = [ffmpeg, "-y", "-i", src]
    
    # Calcular cantidad dinámica igual que antes (uno cada 3 a 6 segundos)
    num_overlays = int(duracion_total / random.uniform(3, 6))
    num_overlays = max(1, num_overlays)
    
    for i in range(num_overlays):
        bloque_tiempo = duracion_total / num_overlays
        t_start = (i * bloque_tiempo) + random.uniform(0.5, bloque_tiempo - 1.0)
